...
计算hash值在数组上下标的映射方法：计算数组下标：`(n - 1)& hash` n是数组长度，开始直接用 hash % n 来取余计算，如果除数n是2的幂次，等价于 (n - 1) & hash, 比如 6 % 4 -> 011 & 101 = 001；效率更高
...
1.7 之前是分段数组 + 链表，对segment加锁每个segment 连以恶HashEntry数组；对同一个segment并发写还是会阻塞，但可以并发写多个segment；默认Segment数组大小16；
...
因为多线程情况下无法区分是key 的value是null 还是 key不存在返回null；比如有key2: null，当线程A put（key2，2）时，还没有替换key2等于2就被线程B get(“key2”) 读走了null；
